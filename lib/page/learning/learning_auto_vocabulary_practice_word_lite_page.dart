...
class _LearningAutoVocabularyPracticeWordLitePage
  ...
  Future<void> sendNextQuestion() async {
    ...
    if (_part > _contentList.length) {
      ...
      var userAnswerRate = _finishQuizData['userAnswerRate'].fold(0, (p, c) => p + c) / _finishQuizData['userAnswerRate'].length;



      await sendChatMessage(false, 'Bot', [
        TextSpan(text: '=== 紀錄 ===\n'),
        TextSpan(text: '答對數/總題數：${ _finishQuizData['scoreArray'].where((score) => score == 100).toList().length } / ${ _finishQuizData['scoreArray'].length } (${ (_finishQuizData['scoreArray'].where((score) => score == 100).toList().length / _finishQuizData['scoreArray'].length  * 100).toStringAsFixed(0) } %)\n'),
