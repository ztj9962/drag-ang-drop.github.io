...
class _VocabularyTestIndexPage extends State<VocabularyTestIndexPage> {
  ...
  void _addAllListData() {
    ...
    _listViews.add(
      const Padding(
          padding: EdgeInsets.all(8),
          child: AutoSizeText('友情提示:為確保結果的準確性，不會的請點擊選項E。',
              maxLines: 2, style: PageTheme.index_vocabulary_test_index_text)),
    );
    _listViews.add(Container(
        ...
        child: Padding(
          ...
          child: Row(
            children: <Widget>[
              Expanded(
                  flex: 9,
                  child: AutoSizeText('1. 常用單字 1~300 字 詞彙測驗',
                      group: titleTextSizeGroup, maxLines: 1)),
              Expanded(flex: 1, child: Icon(Icons.play_arrow, size: 30)),
            ],
          ),
        ),
        onPressed: () {
        ...
        child: Padding(
          ...
          child: Row(
            children: <Widget>[
              Expanded(
                  flex: 9,
                  child: AutoSizeText('2. 常用單字 301~500 字 詞彙測驗',
                      group: titleTextSizeGroup, maxLines: 1)),
              Expanded(flex: 1, child: Icon(Icons.play_arrow, size: 30)),
            ],
          ),
  ...
  Future<void> _jumpQuestingPage(int indexMin, int indexMax) async {
    ...
    try {
      ...
      int doLimit = 1;
      do {
        String responseJSON = await APIUtil.vocabularyTestGetQuestion(
    ...
    AutoRouter.of(context).push(VocabularyTestQuestingRoute(
        vocabularyTestQuestionList: _vocabularyTestQuestionList));
  }
}
